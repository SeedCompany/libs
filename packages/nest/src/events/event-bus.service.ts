...
import { type IEventHandler } from './event-handler.decorator.js';
...
export class SyncEventBus implements IEventBus, OnApplicationBootstrap {
  private listenerMap: Record<string, FnLike[]> = {};

  constructor(
    private readonly discovery: DiscoveryService,
    private readonly logger = new Logger(SyncEventBus.name),
  ) {}
  ...
  async onApplicationBootstrap() {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      for (const entry of discovered) {
        ...
        if (defined.has(name)) {
          throw new InternalServerErrorException(stripIndent`
