...
import { cmpBy } from './sort-by.js';

/**
 * A handle for a certain voter to cast their vote.
 */
export class Ballot<Choice, Voter = unknown> {
  /** @internal */
  constructor(
    readonly voter: Voter,
    private readonly state: PollState<Choice, Voter>,
  ) {}
...
/**
 * A view of a {@link Poll} that allows individual voters to vote.
 */
...
/**
 * A way for multiple decoupled parts of a codebase to give input on a decision.
 *
 * @example
 * const poll = new Poll<boolean>();
 *
...
/**
 * A helper class for {@link PollResult} that declares the winner strategies.
 */
class VoteTally<Choice, Voter> {
  ...
  get plurality() {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- false positive on index access
    if (!highest) {
      return undefined;
    }
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- false positive on index access
